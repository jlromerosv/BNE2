jQuery(document).ready(function ($) {
    ...
    if ($('.path-user').length > 0) {
        ...
        //   $(".form-type-password svg").click(function (e) {
        //     console.log("se ha pulsado el ojo")

        //     let tipo = $(".form-item-pass input").attr("type");
        //     console.log(tipo)
        //     if (tipo == "password") {
        //         console.log("el tipo es password")
        //       $(".form-type-password input").attr("type", "text");
        //     } else {
    ...
    if (window.location.pathname.includes('/user/login')) {
        ...
        $('#block-sede-theme-contenidoprincipaldelapagina h1').text("Acceder")

        //cambia el enlace de reiniciar contraseña para que aparezca en el formulario bajo el input de la contraseña
        $('.ps_microsite_container .region #block-sede-theme-local-tasks nav ul li:last-child a').appendTo('.user-login-form .form-type-password')
        //cambia el texto del enlace reiniciar contraseña
        $(".user-login-form .form-type-password a").text("¿Olvidaste tu Contraseña?");


        $('.ps_microsite_container .region #block-sede-theme-local-tasks nav ul li:last-child').appendTo('.user-login-form .form-actions')

        $('.user-login-form .form-actions').append('<div class="acceso-privado"><div>Acceso privado pra usuarios de la BNE.</div><div><span>Puedes suscribirte a nuestro boletin pinchando</span><a href="/">aquí.</a></div></div>')
    ...
    if (window.location.pathname.includes('/user/register')) {
        $('#block-sede-theme-contenidoprincipaldelapagina h1').text("Crear cuenta de usuario")
    ...
    if (window.location.pathname.includes('/user/password')) {
        //cambia el texto del titulo de la pagina de login
        $('#block-sede-theme-contenidoprincipaldelapagina h1').text("Reiniciar contraseña")
